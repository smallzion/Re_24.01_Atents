...
public class Background_Planet : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(transform.position.x < baseLineX)    //기준선보다 왼쪽으로가면
        {
            transform.position = new Vector3(Random.Range(minRightEnd, maxRightEnd), Random.Range(minY, maxY), 0.0f);   //오른쪽으로랜덤한 거리만큼 이동
            //위 아래도 랜덤으로 조절
        }
    }
}
